...
Since, the publication in 1944, of \textit{"Theory of Games and Economic Behaviour"} from John Von Neumman et Oskar Morgenstern
, theory games knows up today a seething with excitement. However, it's applications are not limited only to political and economical fields. In the seventies, William Hamilton and Robert Trivers will be the first to foresee mathematical ecology, 
or more generally life sciences in Game Theory. However, it will be in 1973, when John Maynard Smith \cite{smith1973The} and George Price, introduce in the paper \textit{Nature} a game theory approach to evolutionnary biology. This has led to an important subfield of Game Theory (and quite disconnected from the rest of it) called Evolutionnary Game Theory. 

This field analyses special types of games where\emph{a large number of identical agents} are playing with \emph{fixed} strategies. Moreover, Evolutionary Game Theory does not assume any rationality of the agents, and these are interacting randomly, reproducing themselves (or disappearing) dependently of their types and the types of their fellow agents. We are rather interested in the different proportions of agents playing different strategies asymptotically. These proportions are to be interpreted as an Equilibrium, in the sense of Game Theory. 
...
like in classical Game Theory) determines the \emph{reproduction rates} of agents. Otherwise, strategies that do well reproduce faster. Strategies that do poorly are outcompeted. This is straightforward borrowed from classical concepts in Ecology : \emph{the natural selection}. Let us see an example: on Figure \ref{frequency} are represented two species A and B; while A is having the ability to move, B is static. Let us assume, that an analysis of cost-benefit lead to a greater fitness when one has the ability to move. Then A will clearly have an advantage on B. This is a constant selection model. Imagine now, that the fitness varies according to the proportion of the species : When there are not many agents in the pool, the agent with the ability to move takes a great advantage of it, and its fitness is high. However, when many agents are present, the agents are bound to be static, whatever their initial capacities are, and the mobile agent looses its comparative advantage. As an illustration, one can imagine a highway where your speed is limited by the law but also by the number of cars in that highway. Thus it makes sense to model the fitness (the rate of reproduction) of the agents as depending on the relative concentrations' of the different agents.  
...
This paradigmatic game in Game Theory comes from Ecology: scientists wanted to understand and explain violence and domination in nature.  Violence can indeed be beneficial for an individual, if it allows him to enhance reproduction of its own genes, compared with other species.  Of course, it can also be harmful when two such individuals meet. In the Hawk and Dove game, we consider two strategies profile, the hawks (H) will exacerbate the conflict, it's the attack strategy. The Doves, conversely, will avoid a confrontation, it's the retreat option. The benefit of winning is the payoff $b$, while the cost of injuries after loosing a fight is $c$. If two hawks strategies meet, there will be a fight. Assuming, they have the same force, the probability for each of them to win is 1/2. The expected payoff is then $(b-c)/2$. If an hawk meets a dove strategy, there will be not fight, as the dove retreats; the hawk have the full b and dove leaves with a zero payoff but with no injuries. Finally, when two strategies dove meet, there will be no injuries, however, we assum that at the end one of them will win with probability 1/2 and the expected payoff is then $b/2$. We thus have the following payoff matrix:
\[
\begin{pmatrix}
	\frac{b-c}{2} & b \\
	0 & \frac{b}{2} 
