...
\begin{definition}[Learning strategy]
    A learning strategy is a strategy where a player use previous information
    to improve its behaviour. As they unfold, learning strategies are supposed
    to draw interesting inferences or use accumulated experience in an
    interesting way.
\end{definition}

There exists a lot of learning strategies but in this course we are only going
to talk about two learning strategies: \emph{Fictitious play} and
\emph{No-regret playing}.
...
$\frac{1}{2}*3+\frac{1}{2}*0=1.5$ and the payoff for playing right is
$\frac{1}{2}*0+\frac{1}{2}*1=0.5$, thus playing $l$ is the best response
...
player 2 is equal to $\frac{1}{3}*3+\frac{2}{3}*0=1$ if he plays l and
$\frac{1}{3}*0+\frac{2}{3}*1=\frac{2}{3}$ if he plays right.\\
...
$\frac{1}{4}*3+\frac{3}{4}*0=0.75$ if he plays l and
$\frac{1}{4}*0+\frac{3}{4}*1=0.75$ if he plays right.\\
...
expected payoff for player 2 is equal to $\frac{1}{5}*3+\frac{4}{5}*0=0.6$
if he plays l and $\frac{1}{5}*0+\frac{4}{5}*1=0.8$ if he plays right.
...
We can thus see thanks to this example that the algorithm of fictious play does
sometimes converge under certain conditions. We are first going to detail the
solution to which the algorithm is going to converge:
...
Once we know this result, we can now detail sufficient conditions for the
convergence of the fictitious play algorithm:
...
\begin{definition}[Regret]
    ...
    $t$\\ Let $\alpha^t(d)$ the average per-period reward the agent would have
    received by playing strategy $d$. \\ The regret an agent experiences at
    time $t$ for not having played $d$ is:
...
matching}. The concept of the regret matching algorithm is the following:
