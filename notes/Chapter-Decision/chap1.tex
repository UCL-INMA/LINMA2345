...
\item Condition \ref{c2} is a formula which establishes how conditional probabilities assessed in one event must be related to conditional probabilities assessed in another. We discuss this in the next section. %Beware that the function $p(\cdot|\cdot)$, which we called a ``\emph{conditional probability function}'', is  only required to satisfy the properties defined in Definition 4, and not all the laws of probability.  The theorem tells us that, on top of Definition 4, it must only satisfy one more law of probability, which is Bayes formula.
...
%TODO In other words,  if we are given a probability distribution $p' \in \Delta(\Omega)$, it is easy to construct a Bayesian probability system (by Bayes rule). The opposite is not true, and we leave it to the reader to find why\footnote{Take $p \in \Delta(\Omega)$, and assume that for a set $\emptyset \neq S \in \Omega$, $p(S) = 0$. Try to compute $p(t|S)$ for $t \in S$}.
