...
In fact, bayesian games are not essentially different from general standard games.  To see this, one can see that, given any bayesian game,  one can build an equivalent standard game, named the 'type-agent representation' of the bayesian game.
 ...
 \end{definition}
  \begin{example}
  The following table shows the payoffs for the type-agent representation of the game of example \ref{ch2:exbayintro}. We assume that the probabilities that Romain has a red or black card are the same.
  These payoffs are to be read
  $$ (\text{Matthew's payoff}/\text{red Romain's payoff}/\text{black Romain's payoff}).$$
  \begin{center}
 \begin{tabular}{l|cc}
 & red:Raise & red:Fold \\
\begin{tabular}{l}
